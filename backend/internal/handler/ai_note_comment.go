...
要求：
1. 每条建议简短自然，像真人刷到笔记随手留言，口语化但不要「小作文」或营销软文腔
2. 不要冒充笔记作者，不要编造事实或效果承诺
3. 避免引流联系方式、违规营销或敏感内容
4. 避免 AI 腔：少用语气词堆砌（如哈哈、嘿嘿、呢呀哦啦、哒、～、连用多个！）；少用空洞万能句（如「太棒了」「绝了」「收藏了」「干货满满」「给我很大启发」）除非与笔记内容强相关；不要套话开场
5. 仅输出严格 JSON：{"suggestions":["评论1","评论2","评论3"]}`, persona)
