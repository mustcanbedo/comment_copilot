...
export default function DashboardPage() {
  ...
  async function fetchComments(user?: UserInfo | null) {
    const u = user ?? me
    ...
    try {
      ...
      const res = await fetch(`/api/comments?${params}`, { headers: { 'x-tenant-id': u.tenantId } })
      const data = await res.json()
  ...
  useEffect(() => {
    ensureAuth().then(user => {
      if (user) fetchComments(user)
    })
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [])

  useEffect(() => {
    if (me) fetchComments(me)
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [filter])
  ...
  async function generateReply(comment: Comment) {
    ...
    try {
      const res = await fetch('/api/ai/reply', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', 'x-tenant-id': me.tenantId },
        body: JSON.stringify({ commentId: comment.id, commentContent: comment.content }),
      })
      const data = await res.json()
      setAiStates(prev => ({
        ...prev,
        [comment.id]: { loading: false, suggestions: data.suggestions ?? [], copied: null, error: data.ok ? undefined : data.error },
      }))
    } catch {
  ...
  async function copyText(commentId: string, text: string, index: number) {
    ...
    setAiStates(prev => ({ ...prev, [commentId]: { ...prev[commentId], copied: index } }))
    setTimeout(() => {
      setAiStates(prev => ({ ...prev, [commentId]: { ...prev[commentId], copied: null } }))
    }, 2000)
  }
    ...
    <div className="min-h-screen bg-gray-50">
      <header className="bg-white border-b border-gray-200 px-6 py-4 flex items-center justify-between">
        ...
        <div className="flex items-center gap-3">
          ...
          <Link href="/settings" className="text-sm px-3 py-1.5 border border-gray-200 rounded-lg hover:bg-gray-50 transition">设置</Link>
          <button onClick={logout} className="text-sm px-3 py-1.5 border border-gray-200 rounded-lg hover:bg-red-50 hover:text-red-500 hover:border-red-200 transition">退出</button>
          <button onClick={() => fetchComments()} className="text-sm px-4 py-1.5 bg-rose-500 text-white rounded-lg hover:bg-rose-600 transition">刷新</button>
        </div>
      </header>
      ...
      <div className="max-w-5xl mx-auto px-6 py-6">
          ...
          {(['all', 'hot', 'warm', 'pending'] as const).map(f => (
            <button
              ...
              className={`px-4 py-1.5 rounded-full text-sm font-medium transition ${
                filter === f ? 'bg-rose-500 text-white' : 'bg-white border border-gray-200 text-gray-600 hover:bg-gray-50'
              }`}
            >
        ...
        {loading ? (
          ...
          <div className="text-center py-16 text-gray-400">暂无评论</div>
        ) : (
                ...
                <div key={comment.id} className="bg-white rounded-xl border border-gray-100 shadow-sm p-4">
                  <div className="flex items-center justify-between mb-2">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-xs text-gray-400">{statusConfig[comment.status] ?? comment.status}</span>
                    </div>
                  </div>
                  ...
                  {!ai && (
                    <button onClick={() => generateReply(comment)} className="w-full py-2 bg-gradient-to-r from-rose-500 to-pink-500 text-white text-sm rounded-lg font-medium hover:opacity-90 transition">
                      生成 AI 回复
                    </button>
                  )}

                  {ai?.loading && <div className="text-center text-sm text-gray-400 py-2">AI 思考中...</div>}
                  {ai?.error && <div className="text-center text-sm text-red-400 py-2">{ai.error}</div>}

                  {ai && !ai.loading && ai.suggestions.length > 0 && (
                    <div className="flex flex-col gap-2 mt-1">
                      {ai.suggestions.map((s, i) => (
                        <div key={i} className="flex items-start gap-2 bg-gray-50 border border-gray-100 rounded-lg p-3">
                          ...
                          <button
                            ...
                            className={`shrink-0 text-xs px-3 py-1 rounded-md border transition ${
                              ai.copied === i ? 'bg-green-50 border-green-200 text-green-600' : 'bg-white border-gray-200 text-gray-500 hover:bg-gray-100'
                            }`}
                          >
                            {ai.copied === i ? '已复制' : '复制'}
                          </button>
